...
class AnimationState:
	"""
	The base interface class of the server game object animation states
	"""
	def __init__(self):
	...
	def onStateChanged_(self, oldState, newState):
		...
		if self.isState(GlobalDefine.ENTITY_STATE_REST or GlobalDefine.ENTITY_STATE_FREE or GlobalDefine.ENTITY_STATE_UNKNOWN):
			self.changeAnimationState(GlobalDefine.ENTITY_ANIMATION_STATE_IDLE)
	...
	def onSubStateChanged_(self, oldSubState, newSubState):
		...
		Substate changed
		"""
		if self.isState(GlobalDefine.ENTITY_SUB_STATE_GO_BACK or GlobalDefine.ENTITY_SUB_STATE_FLEE or GlobalDefine.ENTITY_SUB_STATE_CHASE_TARGET):
