...
class ChannelAbility(ActiveAbility):
	def __init__(self):
	...
	def onStartChanneling(self, caster, receiver):
		DEBUG_MSG('got it!')
	...
	def onChannelingTick(self, caster, receiver):
		DEBUG_MSG('tick')

	def onStopChanneling(self, caster, receiver):
	...
	def initiateClientChannelCancel(self, caster):
		if caster.allClients:
	...
	def initiateClientsCasting(self, caster):
		if caster.allClients:
			caster.allClients.onAbilityStateChange(GlobalDefine.ENTITY_ABILITY_STATE_CHANNELING, self.getID(), self.getTargetID(), self.getDisplayID(), float(self.getCastTime()))
